...
\begin{abstract}
The developments in robotics technology motivate the need for stable and reliable autonomous robots. Using the \textit{ROS} framework and the \textit{STDR Simulator} package a wall following robot was design and implemented. The robot's design takes into consideration the specifications and restrictions of the problem the robot has to face, follow indefinitely a wall as soon it is discover. A set of maps where used to test the behaviour and the stability of the robot during its movement. From our tests, we observed that the robot maintained a stable movement for most of the configurations. The only case, we see a more disturbed movement, its when it has to make a turn of 90º degrees from the outside of the wall because the wall "disappear" from the sensor and the robot has to adjust it's trajectory until he regains sight of the wall.
\end{abstract}
